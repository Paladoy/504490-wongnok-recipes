...
const newEmailIsInvalid = computed(() => {
  if (!newEmail.value || !newEmailConfirm.value || newEmail.value !== newEmailConfirm.value) {
    return true;
  }
  return false;
});
...
<template>
  ...
  <v-row>
    ...
    <v-col cols="6" offset="6">
      <v-btn block @click="authStore.updateEmail(newEmail)" :disabled="newEmailIsInvalid">
        Update Email
      </v-btn>
    </v-col>
  </v-row>
